...
object helpers {
  ...
  final case class CRecord(name: Option[String], fields: Map[String, CType]) extends CType
  ...
  def updateNs(ns: Option[String]): ResultS[Unit] = liftS(State.modify[ParserState](_.copy(ns = ns)))
  def pushNode(n: Node): ResultS[Unit] = liftS(State.modify[ParserState](s => s.copy(path = xml.push(s.path, n))))
  def popNode: ResultS[Unit] = liftS(State.modify[ParserState](s => s.copy(path = xml.pop(s.path))))
  ...
  def attr(name: String)(node: Node): ResultS[String] = liftE(xml.getAttr(name)(node))
  def attrO(name: String)(node: Node): ResultS[Option[String]] = right(xml.getAttrO(name)(node))
  def el(name: String)(node: Node): ResultS[Node] = getNs.flatMap(ns => liftE(xml.getByName(name, ns)(node)))
  def elO(name: String)(node: Node): ResultS[Option[Node]] = getNs.flatMap(ns => right(xml.getByNameO(name, ns)(node)))
  def els(name: String)(node: Node): ResultS[List[Node]] = getNs.flatMap(ns => right(xml.findByName(name, ns)(node)))
  def intAttr(name: String)(node: Node): ResultS[Int] = attr(name)(node).flatMap(v => liftE(Try(Integer.parseInt(v)).toEither))
  def optElAttrAsInt(name: String, attrName: String)(node: Node): ResultS[Option[Int]] =
